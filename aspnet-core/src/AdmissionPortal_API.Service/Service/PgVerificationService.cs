...
{
    public class PgVerificationService : IPgVerificationService
    {
        private readonly IPgVerificationRepository _verificationRepository;
        private readonly IMapper _mapper;
        private readonly IConfiguration _configuration;
        private readonly IBlobService _blobService;
        private readonly ILogError _logError;
        public PgVerificationService(IPgVerificationRepository verificationRepository, IMapper mapper, IConfiguration configuration, ILogError logError, IBlobService blobService)
        {
            _logError = logError;
        ...
        public async Task<ServiceResult> GetVerifiedStudentByRegIdCollege(string RegId, int CollegeId)
        {
            ...
            catch (Exception ex)
            {
                ...
                serviceResult.Status = false;
                serviceResult.StatusCode = Convert.ToInt32(HttpStatusCode.BadRequest);
                _logError.WriteTextToFile("Get Verified PG Student by RegId and College : ", ex.Message, ex.HResult, ex.StackTrace); ;
            }
        ...
        public async Task<ServiceResult> VerifyStudentWithSection(PgVerifyStudentWithSection model)
        {
            ServiceResult serviceResult = new ServiceResult();
            try
            {

                var obj = _mapper.Map<PgVerifyStudentWithSection>(model);
        ...
        public async Task<ServiceResult> GetPGStudentByRegId(string RegId, Int32 type)
        {
            ServiceResult serviceResult = new ServiceResult();
            try
            {
                var _result = await _verificationRepository.GetPGStudentByRegId(RegId, type);
        ...
        public async Task<ServiceResult> UnlockStudentVerification(CancelStudentRegistration model)
        {
            ServiceResult serviceResult = new ServiceResult();
            try
            {
                //var obj = _mapper.Map<VerifyStudentWithSection>(model);
                var _result = _verificationRepository.UnlockStudentVerification(model);
        ...
        public async Task<ServiceResult> StudentCourseEligible(PGStudentChoiceofCourseEligible model)
        {
            ServiceResult serviceResult = new ServiceResult();
            try
            {
        ...
        public async Task<ServiceResult> FinalSubmit(PGFinalVerificationModel model)
        {
            ServiceResult serviceResult = new ServiceResult();
            try
            {
