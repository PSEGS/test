...
{
    [Route("api/meritmoduleug")]
    [Authorize(AuthenticationSchemes = Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerDefaults.AuthenticationScheme)]
    public class MeritModuleUGController : ControllerBase
    {
        private readonly IMeritModuleUGService _meritService;
        private readonly ILogError _logError;
        public MeritModuleUGController(IMeritModuleUGService meritService, ILogError logError)
        {
        ...
        public async Task<IActionResult> SaveAdmissionSeat([FromBody] AdmissionSeat seat)
        {
            try
            {
                ...
                if (identity != null)
                {
                    ...
                    universityId = Convert.ToInt32(identity.FindFirst("headId").Value);
                }
                //if (universityId == 63 && seat.CourseId == 1)
                //{
                //    data.Message = "Seat Offer module for BCOM course is temporarily on hold for Panjab University Chandigarh.";
                //    data.ResultData = null;
                //    data.Status = false;
                //    return await Task.Run(() => Ok(data));
                //}
                data = await _meritService.SaveAdmissionSeat(seat);
                if (data.Status == true)
                {
                    return await Task.Run(() => Ok(data));
                }
        ...
        [HttpGet]
        [Route("GetCourseFeeByStudentId/{CourseId}/{CollegeId}/{StudentId}/{Combination}")]
        public async Task<IActionResult> GetCourseFeeByStudentId(int CourseId, int CollegeId, int StudentId, string Combination)
        {
            try
            {

                ServiceResult data = await _meritService.GetCourseFeeByStudentId(CourseId, CollegeId, StudentId, Combination);
                if (data.Status == true)
                {
                    return await Task.Run(() => Ok(data));
                }
        ...
        [HttpGet]
        [Route("GetCourseChoiceByRegId/{RegId}/{CourseId}/{CollegeId}")]
        public async Task<IActionResult> GetCourseChoiceByRegId(string RegId, Int32 CourseId, Int32 CollegeId)
        {
            try
            {
                ServiceResult data = await _meritService.GetCourseChoiceByRegId(RegId, CourseId, CollegeId);
                if (data.Status == true)
                {
                    return await Task.Run(() => Ok(data));
                }
        ...
        [HttpGet]
        [AllowAnonymous]
        [Route("GetCategoryCombination")]
        public async Task<IActionResult> GetCategoryCombination()
        {
            try
            {
                ServiceResult data = await _meritService.GetCategoryCombination();
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    List<ExportExcelModel> exportColumns = new()
                    {
                        ...
                        new ExportExcelModel { Column = "TwelvethPercentage", ColumnLabel = "12th" },
        ...
        public async Task<IActionResult> VerifyBookSeatOTP([FromBody] VerifyOTP model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ServiceResult data = new ServiceResult();
                    //int universityId = 0;
                    //if (universityId == 63 && model.CourseId == 1)
                    //{
                    //    data.Message = "Seat Book module for BCOM course is temporarily on hold for Panjab University Chandigarh.";
                    //    data.ResultData = null;
                    //    data.Status = false;
                    //    return await Task.Run(() => Ok(data));
                    //}
                    data = _meritService.VerifyOTP(model);
                    if (data.StatusCode == 200)
                    {
                        return await Task.Run(() => Ok(data));
                    }
        ...
        [HttpGet]
        [Route("CollegeCourseSubjectCount/{collegeId}/{courseId}")]
        public async Task<IActionResult> CollegeCourseSubjectCount(string collegeId, string courseId)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ServiceResult data = await _meritService.CollegeCourseSubjectCount(collegeId, courseId);
                    if (data.StatusCode == 200)
                    {
                        return await Task.Run(() => Ok(data));
                    }
        ...
        public async Task<IActionResult> RevokeAdmissionSeat([FromBody] RevokeSeat revoke)
        {
            ...
            {
                _logError.WriteTextToFile("Revoke admission seat UG: ", ex.Message, ex.HResult, ex.StackTrace);
                return BadRequest(ex);
            }
        }
        ...
        [HttpGet]
        [Route("GetVacantSeatMatrixByCategory/{collegeId}/{courseId}/{CombinationId}")]
        public async Task<IActionResult> GetVacantSeatMatrixByCategory(int? collegeId, int courseId, int combinationId)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    ServiceResult data = await _meritService.GetVacantSeatMatrixByCategory(collegeId, courseId, combinationId);
                    if (data.StatusCode == 200)
                    {
                        return await Task.Run(() => Ok(data));
                    }
        ...
        [HttpPost]
        [Route("UpdateBookedSeatMatrix")]
        public async Task<IActionResult> UpdateBookedSeatMatrix([FromBody] UpdateBookedSeatMatrix model)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                ServiceResult data = await _meritService.UpdateBookedSeatMatrix(model);
                if (data.Status == true)
                {
                    return await Task.Run(() => Ok(data));
                }
